...
When you need to solve a partial differential equation — whether it's heat diffusion, electrostatics, fluid flow, or stress analysis — the standard approach is to **discretize the entire domain**. Finite element methods (FEM), finite difference methods (FDM), and their cousins break your problem into millions of grid points or elements, then solve a massive linear system.

This works. But it has two fundamental limitations:

1. **You pay for the whole domain** — even if you only care about the solution at a few points
...
## The Insight: You Don't Need the Whole Domain
...
Harmonic functions — solutions to Laplace's equation ∇²u = 0 — have a beautiful property called the **mean value property**: the value at any point equals the average value over any sphere centered at that point.

```
u(x) = (1/|∂B|) ∫_{∂B(x,r)} u(y) dS(y)
```

In plain English: the temperature at a point in a steady-state heat distribution equals the average temperature on any sphere around it.

This seems almost too good to be true. But it's mathematically rigorous, and it leads directly to a Monte Carlo algorithm.
...
- **No mesh generation** — Complex geometries? No problem. The algorithm only needs to know "am I inside or outside?"
...
For **time-dependent problems** like the heat equation, you can use WoS in the spatial dimensions and combine it with time-stepping, or use space-time random walks for certain formulations.
...
A Walk-on-Spheres simulation with 10 million random walks? **Seconds on a consumer GPU.**
...
I've been working on a GPU-accelerated WoS implementation using NVIDIA Warp, and the results are striking. Problems that would take hours with FEM run in minutes. And because the method is Monte Carlo, you can **stream results** — get a rough answer quickly, then refine it as more walks complete.
...
What excites me about Walk-on-Spheres isn't just that it's faster — it's that it **changes the computational paradigm**. Instead of "discretize everything, solve the big system," you ask "what do I actually need to know, and how can I get just that?"
...
I'm currently building an open-source GPU-accelerated WoS library. The goal is to make this method accessible to anyone who needs to solve PDEs — no PhD in numerical analysis required.

Stay tuned. The code will be on GitHub soon, along with tutorials showing how to solve real problems with random walks.

---

*This method is described in detail in the pedagogical paper available on [Overleaf](https://www.overleaf.com/project/6a1117df1fa857369e98000c), with slides covering the theoretical foundations and practical implementation.*
